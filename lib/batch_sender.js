 ...
 *
 * @param {String|Array} toEmail	a string containing the message recipient's email address - OR - an options list
 ...
 * @param {Object=} options			Required. any and all optional parameters
...
BatchSender.prototype.push = function(toEmail, fromEmail, subject, options, cb) {
	if (_.isArray(arguments[0])) {
		toEmail = arguments[0].toEmail;
		fromEmail = arguments[0].fromEmail;
		subject = arguments[0].subject;
		cb = arguments[0].cb;

		delete arguments[0]["toEmail"];
		delete arguments[0]["fromEmail"];
		delete arguments[0]["subject"];
		delete arguments[0]["cb"];
		options = arguments[0];
	}
